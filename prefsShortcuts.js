...
export function initShortcutRows(listBox, settings) {
    for (const [title, description, settingsKey] of [
        [_('Save Session'), _('Uses the restore-at-startup session.'), 'save-session-shortcut'],
        [_('Restore Session'), _('Uses the restore-at-startup session.'), 'restore-session-shortcut'],
        [_('Move Windows'), _('Uses the restore-at-startup session.'), 'move-windows-shortcut'],
    ]) {
